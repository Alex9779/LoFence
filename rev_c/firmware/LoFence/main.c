...
void rn2483_init() {
	...
	sprintf(buffer_rn, "mac set pwridx %u\r\n", PWRIDX);
	rn2483_tx(buffer_rn);
	rn2483_rx();
	...
	sprintf(buffer_rn, "mac set dr %u\r\n", DR);
	rn2483_tx(buffer_rn);
	rn2483_rx();
	if (strcmp(buffer_rn, "ok\r\n") != 0) {
		rn2483_tx_error();
	...
	#ifdef ADR
	sprintf(buffer_rn, "mac set adr on\r\n");
	rn2483_tx(buffer_rn);
	rn2483_rx();
	if (strcmp(buffer_rn, "ok\r\n") != 0) {
		rn2483_tx_error();
...
void transmit() {
	...
	sprintf (buffer_rn, "mac tx uncnf 1 %04X%04X%04X\r\n", volt_bat, volt_fence_plus, volt_fence_minus);
	rn2483_tx(buffer_rn);
	rn2483_rx();
	if (strcmp(buffer_rn, "ok\r\n") != 0) {
		rn2483_tx_error();
...
int main(void) {
	...
	debug("\r\n");
	debug("\r\n");
	debug("LoFence REV C v0.6 by alex9779\r\n");
	debug("https://github.com/alex9779/lofence\r\n");
